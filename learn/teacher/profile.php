...
$query=mysql_query("update enrollteacher set teachername='$studentname',email='$email',phoneno='$phoneno' where username='".$_SESSION['login']."'");
if($query)
{
  echo '<script> alert("Teacher details has Updated successfully")</script>';
}
else
{
...
<html lang="en">
 ...
 <?php 
 $query=mysql_query("select * from enrollteacher  where username='".$_SESSION['login']."'");
 while($row=mysql_fetch_array($query)) 
 {
 ?>                     
 <form class="form-horizontal style-form" method="post" name="profile" >

  <h4 class="mb"><i class="fa fa-user"></i>&nbsp;&nbsp;<?php echo htmlentities($row['teachername']);?>'s Profile</h4>
    
   
<label class="col-sm-2 col-sm-2 control-label">Full Name</label>
<div class="col-sm-4">
<input type="text" name="teachername" required="required" value="<?php echo htmlentities($row['teachername']);?>" class="form-control" >
 </div>
  
 <div class="form-group">
<label class="col-sm-2 col-sm-2 control-label">Departement Name </label>
 <div class="col-sm-4">
<input type="text" name="address" required="required" value="<?php echo htmlentities($row['Address']);?>" class="form-control" readonly>
</div>
</div>
